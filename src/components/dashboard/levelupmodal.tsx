...
import { Zap, Trophy, Star, Sparkles, X } from 'lucide-react';
...
interface LevelConfig {
  name: string;
  color: string;  // ex: 'text-purple-400'
  bg: string;     // ex: 'bg-purple-400/10'
  border: string; // ex: 'border-purple-400/20'
}
...
export function LevelUpModal({ isOpen, onClose, levelData }: LevelUpModalProps) {
  // Guard antes de qualquer hook ou desestruturação
  if (!levelData) return null;

  const { name, color, bg, border } = levelData;

  // Glow de fundo: converte 'text-purple-400' → 'bg-purple-400'
  // Feito com split para evitar replace frágil em strings compostas
  const glowBg = color.startsWith('text-') ? color.replace('text-', 'bg-') : bg;
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 opacity-20 rounded-full blur-[80px] -z-10 pointer-events-none ${glowBg}`} />
            ...
            <div className="flex justify-center mb-8">
              <div className="relative">
                ...
                >
                  <Trophy size={56} className={color} />
                </motion.div>
            ...
            >
              ...
              <p className="text-slate-400 mb-8 leading-relaxed font-medium">
                Sua dedicação está valendo. Continue estudando para chegar ao topo do ranking!
              </p>

              {/* Badge do nível */}
              <div className={`${bg} ${border} border p-5 rounded-[2rem] mb-8`}>
                ...
                <h3 className={`text-2xl font-black flex items-center justify-center gap-3 ${color}`}>
                  <Zap className="fill-current flex-shrink-0" size={24} />
            ...
            <motion.div
              ...
              className={`absolute top-10 left-10 ${color} pointer-events-none`}
            >
            ...
            <motion.div
              ...
              className={`absolute bottom-20 right-10 ${color} pointer-events-none`}
            >
