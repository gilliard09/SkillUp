...
import { MobileNav } from '@/components/MobileNav'; // Importando o novo componente
...
export default function ProfilePage() {
  ...
  async function fetchProfileData() {
    try {
      ...
      const courseIds = enrollments?.map(e => e.product_id) || [];
      const { data: modules } = await supabase.from('modules').select('id').in('course_id', courseIds);
      const moduleIds = modules?.map(m => m.id) || [];

      const { count: totalCount } = await supabase.from('lessons').select('*', { count: 'exact', head: true }).in('module_id', moduleIds);
      const { count: completedCount } = await supabase.from('lesson_progress').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
  ...
  return (
    // pb-32 garante que o conteúdo não fique atrás da MobileNav no celular
    <div className="min-h-screen pb-32 bg-slate-0 px-4 md:px-10 lg:px-20 max-w-7xl mx-auto">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-slate-500 text-sm mb-6 font-medium">Como você deseja ser chamado na plataforma?</p>

              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest font-black text-brand-primary ml-2">Nome Completo</label>
                  <input 
                    type="text" 
                    value={newName}
          ...
          <div className="text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-2">
              <span className="flex items-center gap-2 text-slate-400 text-xs font-medium">
                <Mail size={14} className="text-brand-primary" /> {profile?.email}
              </span>
              <span className="flex items-center gap-2 text-slate-400 text-xs font-medium">
          ...
          <div className="bg-slate-900/40 border border-white/5 p-6 rounded-3xl backdrop-blur-md relative overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-slate-500 text-xs font-medium">Concluído: {stats.completedLessons} / {stats.totalLessons} aulas</p>
              </div>
              <span className="text-3xl font-black text-brand-primary italic">{stats.progressPercentage}%</span>
            </div>
            ...
            <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden mb-6 border border-white/5">
              <motion.div 
                ...
                className="h-full bg-brand-primary shadow-[0_0_15px_rgba(255,107,0,0.3)]"
              />
            </div>
            ...
            {stats.progressPercentage >= 100 ? (
              ...
              <p className="text-[10px] text-slate-600 uppercase font-bold text-center tracking-widest">Complete 100% para o certificado</p>
            )}
          </div>

          {/* XP e Nível */}
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-slate-900/40 border border-white/5 p-6 rounded-3xl flex flex-col items-center text-center group transition-all">
              ...
              <p className="text-2xl font-black text-white italic">{profile?.xp?.toLocaleString() || 0}</p>
            </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="p-6 bg-gradient-to-br from-brand-primary to-brand-secondary rounded-3xl relative overflow-hidden group shadow-xl shadow-brand-primary/20">
            <div className="relative z-10">
              <h3 className="text-white font-black text-xl italic uppercase leading-tight">Indique amigos e<br/>GANHE PRÊMIOS!</h3>
              <a 
                href={COMMUNITY_LINK}
                target="_blank"
            ...
            <Trophy size={80} className="absolute -right-4 -bottom-4 opacity-20 rotate-12 text-white group-hover:scale-110 transition-transform" />
          </div>
        </div>
      </div>
